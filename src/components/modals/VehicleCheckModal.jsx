...
import CarDiagram2D, { DOT_POS, ZONE_LABELS } from '@/components/vehicles/CarDiagram2D';
...
const VehicleCheckModal = ({ isOpen, onClose, type, vehicle, onConfirm }) => {
  ...
  const handleZoneSelect = (zoneId) => {
    if (!zoneId) return;
    const existing = damages.find(d => d.zoneId === zoneId && d.type === selectedDamageType);
    if (existing) {
      setDamages(damages.filter(d => d.id !== existing.id));
    } else {
      const dot = DOT_POS[zoneId] || { x: 150, y: 300 };
      setDamages([...damages, {
        id: Date.now(),
        zoneId,
        x: (dot.x / 300) * 100,
        ...
        description: `${damageTypes[selectedDamageType].label} — ${ZONE_LABELS[zoneId] || zoneId}`,
      }]);
    }
  };
          ...
          <div className="flex-1 overflow-y-auto bg-gray-50/30">
            <div className="grid grid-cols-1 lg:grid-cols-12 h-full">
              ...
              <div className="lg:col-span-8 p-8 flex flex-col bg-gray-50/50">
                ...
                {/* Main Diagram Area */}
                <div className="flex-1 bg-white rounded-2xl border border-gray-200 shadow-sm p-6 relative flex flex-col items-center justify-center overflow-hidden">
                  <div className="absolute top-4 left-4 text-xs text-gray-400 bg-gray-50 px-3 py-1.5 rounded-full border border-gray-100">
                    {hoveredZone ? `Zone : ${ZONE_LABELS[hoveredZone]}` : 'Cliquez une zone pour signaler un dégât'}
                  </div>
                  ...
                  <div className="relative">
                    <CarDiagram2D
                      ...
                      issuePartIds={damages.map(d => d.zoneId).filter(Boolean)}
                      width={280}
                    />
                    {damages.map((damage, index) => {
                      ...
                      return (
                        <motion.div
                          ...
                          style={{ left: `${damage.x * 2.8}px`, top: `${damage.y * 5.6}px`, transform: 'translate(-50%,-50%)' }}
                        >
                          <div className={`w-6 h-6 rounded-full border-2 shadow-md flex items-center justify-center cursor-pointer ${typeConfig.color} ${typeConfig.border}`}
